...
<div class="py-120">
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <a href="{{route('admin.roles.index')}}" class="inline-flex items-center px-4 py-2 
            ...
            <div class="w-full  md:max-w-full mx-auto">
                <div class="p-6 border border-gray-300 sm:rounded-md">
                 ...
                 @endif
                    ...
                    <label class="block mb-6">
                        <span class="text-gray-700 mb-4 block">Lista de Permisos
                            @error('permissions')
                        <span class="text-sm text-red-600" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                    @enderror
                        ...
                        <div class="flex justify-between">
                            <div class="w-1/3">
                                @foreach ($permissions->chunk(ceil($permissions->count() / 3))[0] as $permiso)
                                
                                    <x-checkbox name="permissions[]" value="{{$permiso->id}}" :checked="in_array($permiso->id, old('permissions', [])) || $role->permissions->contains($permiso->id) "                                       
                                        label="{{$permiso->description}}" wire:model.defer="model" />
                                @endforeach
                            </div>
                        
                            <div class="w-1/3">
                                @foreach ($permissions->chunk(ceil($permissions->count() / 3))[1] as $permiso)
                                    <x-checkbox name="permissions[]" value="{{$permiso->id}}" :checked="in_array($permiso->id, old('permissions', [])) || $role->permissions->contains($permiso->id) "
                                         label="{{$permiso->description}}" wire:model.defer="model" />
                                @endforeach
                            </div>
                            ...
                            <div class="w-1/3">
                                @foreach ($permissions->chunk(ceil($permissions->count() / 3))[2] as $permiso)
                                    <x-checkbox name="permissions[]" value="{{$permiso->id}}" :checked="in_array($permiso->id, old('permissions', [])) || $role->permissions->contains($permiso->id) "
                                        label="{{$permiso->description}}" wire:model.defer="model" />
                                @endforeach
                            </div>

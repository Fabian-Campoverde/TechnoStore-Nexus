<div>
    ...
    <input wire:model="name" wire:keydown.debounce.500ms="search" type="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="" >

    @if ($a === false)
        <p class="text-sm text-red-600">El nombre ya está en uso.</p>
    @endif --}}
...
<div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
    ...
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
        ...
        @if ($users->id)
        action="{{route('admin.users.update',['user'=>$users->id])}}"
        @else
        ...
        @endif
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="sm:col-span-2">
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nombres completos</label>
                    <input type="text" value="{{old('name',$users->name)}}" wire:model="name" wire:keydown.debounce.500ms="search"
                ...
                <div class="sm:col-span-2">
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Correo Electronico</label>
                    <input type="email" value="{{old('email',$users->email)}}" wire:model="email" wire:keydown.debounce.500ms="search"
                ...
                <div class="w-full">
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nickname</label>
                    <input type="text" value="{{old('nickname',$users->nickname)}}" wire:model="nickname" wire:keydown.debounce.500ms="search"
                ...
                <div class="w-full">
                    ...
                    <input type="text" value="{{old('password',$users->password)}}" wire:model="password" wire:keydown.debounce.500ms="search"
                     name="password" id="password" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"  >
                    @error('password')
                    <span class=" text-sm text-red-600" role="alert">
                        <strong>{{$message}}</strong>
                    @enderror
                ...
                <label class="block mb-6">
                    ...
                    @endif --}}
                    @if ($users->id)
                                <img class="w-10 h-10 rounded-full" src="{{asset($users->image_url)}}"/>
                                @endif
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Imagen</label>
                    <input name="image_url" type="file" wire:model="image_url" wire:keydown.debounce.500ms="search"
                        class="
                  block
                  w-full
                  mt-1
                    ...
                    <ul class="w-48 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                        ...
                        <li class="w-full border-b border-gray-200 rounded-t-lg dark:border-gray-600">
                            <div class="flex items-center ps-3">
                                <input name="roles[]" type="checkbox" value="{{ $role->id }}" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
            ...
            <button type="submit" @if (!$formIsValid) disabled @endif

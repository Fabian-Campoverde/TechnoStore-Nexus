...
<div class="bg-sky-500 p-4 text-center rounded-lg shadow-lg">
    <h1 class="text-2xl font-semibold text-white uppercase">Registro de Venta de Productos</h1>
</div>
@endsection
...
<style>
 ...
 #clientTypeSwitcher:checked + .client-type-switcher-label .text-primary {
        display: none;
    }

    #clientTypeSwitcher:checked + .client-type-switcher-label .text-body-color {
        display: flex;
    }

    #clientTypeSwitcher:not(:checked) + .client-type-switcher-label .text-body-color {
        display: none;
    }
 </style>
...
focus:ring-offset-2 transition ease-in-out duration-150 ml-4">
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
            ...
            <h2 class="mb-4 text-xl font-bold text-center text-gray-900">Venta de productos</h2>
<form @if ($sale->id)
    action="{{route('admin.sales.update',['sale'=>$sale->id])}}"
    @else
    ...
    @if ($sale->id)
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="w-full">
                        <label for="date" class="block mb-2 text-sm font-medium text-gray-900">Fecha de
                            Emision</label>
                        <div class="relative ">
                            <input autocomplete="off" name="fechaEmision" id="fechaEmision" datepicker datepicker-format="yyyy/mm/dd" type="text"
                                ...
                                placeholder="Selecciona fecha" value="{{ old('fechaEmision', $sale->fechaEmision ?? now()->format('Y-m-d') ) }}">
                    ...
                    <div class="flex items-center space-x-4">
                        <div class="w-1/2">
                            ...
                            <input disabled type="text" name="serie" id="serie" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required="">
                        </div>
                        
                        <div class="w-1/2">
                            <label for="correlativo" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Correlativo</label>
                            <input disabled type="number"  name="correlativo" id="correlativo" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required="">
                        </div>
                    </div>
                    <div class="w-full">
                        <label for="vendedor" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Vendedor</label>
                        <input disabled value="{{ auth()->user()->name ?? '' }}" type="text" name="vendedor_id" id="vendedor_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required="">
                    </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="w-full">
                <input type="checkbox" name="clientTypeSwitcher" id="clientTypeSwitcher" class="sr-only">

<label for="clientTypeSwitcher" class="client-type-switcher-label relative inline-flex cursor-pointer select-none items-center justify-center rounded-md bg-white dark:bg-dark-2 p-1">
    <span class="text-primary bg-gray dark:bg-dark-3 flex items-center space-x-[6px] rounded py-2 px-[18px] text-sm font-medium">
        <!-- Contenido para Nuevo Cliente -->
        Nuevo Cliente
    </span>
    <span class="text-body-color flex items-center space-x-[6px] rounded py-2 px-[18px] text-sm font-medium">
        <!-- Contenido para Cliente Registrado -->
        Cliente Registrado
    </span>
</label>
            </div>
...
<div class="grid gap-6 mb-6 md:grid-cols-3">
    <div>
        ...
        <label for="number-sale" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Cantidad</label>
...
<script>
    $(document).ready(function() {
        var fecha = new Date();
        document.getElementById("fechaEmision").value = fecha.toJSON().slice(0,10);

        // Manejar el cambio en el campo de selección "Comprobante"
        $('select[name="invoice_id"]').change(function() {
            // Obtener el valor seleccionado
            var selectedInvoiceId = $(this).val();
    
            // Obtener la opción seleccionada
            var selectedOption = $('select[name="invoice_id"] option:selected');
    
            // Obtener el valor de 'serie' y 'correlativo' de la opción seleccionada
            var serie = selectedOption.data('serie') || '';
            var correlativo = selectedOption.data('correlativo') || '';
    
            // Llenar automáticamente los campos 'serie' y 'correlativo'
            $('#serie').val(serie);
...
<script>
    $(document).ready(function() {
        $('#cantidad, #precio').on('sale', function() {
...
//                     html:
//                     '<label for="swal-sale-cantidad">Cantidad:</label>' +
//                     '<sale id="swal-sale-cantidad" class="swal2-sale" value="' + cantidad + '">' +
//                     '<label for="swal-sale-precio">Precio:</label>' +
//                     '<sale id="swal-sale-precio" class="swal2-sale" value="' + precio + '">',
...
//                     preConfirm: () => {
//                         return [
//                             $('#swal-sale-cantidad').val(),
//                             $('#swal-sale-precio').val()
//                         ];
//                     }
//                 }).then((result) => {
